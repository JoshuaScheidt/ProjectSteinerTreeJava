...
public class UndirectedGraph {
    ...
    /**
     * This Method removes any edges and vertices not contained within the
     * remainder List. This is to check if a solution is correct (in the matter
     * of connectivity not optimality)
     *
     * @param remainder Your resulting edge list that you want to have in the
     * graph
     */
    ...
    /**
     * Method for constructing edges using only keys. It checks if these keys
     * have been used and else will use the initialised Vertex If neither has
     * been initialised this will be done
     *
     ...
     * connection and removing all of these edges from its neighbours If the
     * removal of an edge creates a disconnected component that neighbour will
     * also be removed
     *
     * @param v The key the to be removed vertex has
     */
        ...
        for (Edge e : toBeRemoved) {
            ...
            if (neighbour.getNeighbors().size() == 1) {
                if (neighbour.isTerminal()) {
                    this.terminals.remove(neighbour.getKey());
                }
                neighbour = null;
            } else {
    ...
    /**
     * Returns all the partial graphs from each separate component in this
     * graph. If the graph is connected, it will simply return the complete
     * graph after performing a complete search.
     *
     ...
     *
     * @param addable the to be added graph
     *
     * @author Joshua Scheidt
     */
